...
import {
  ContainerOAuth, FloatingLabelInputContainer,
  FloatingLabel, OAuthDescription, Form, QRCodeImage
} from './styles';
...
function QRCode() {
  ...
  const handleTFACode = () => {
    ...
    const secret = state.auth && state.auth.qrCode && state.auth.qrCode.secret && state.auth.qrCode.secret.ascii
  ...
  return (
    <>
      {oAuthSuccess ? (
        <ContainerOAuth>
          <OAuthDescription>Digite o código que aparece em seu app Google Authenticator</OAuthDescription>
          <FloatingLabelInputContainer>
            <Form ref={formRef} onSubmit={handleTFACode} id="editForm">
              <FloatingLabel
                htmlFor="OAuthCode"
                active={inputActive.OAuthCode}
              >
          ...
          <ContainerOAuth style={{height: 320}}>
            <OAuthDescription>Escaneie o QRCode em seu celular usando o App Google Authenticator</OAuthDescription>
            <QRCodeImage src={state.auth.qrCode.data} alt="QRCODE" />
            <OAuthDescription>Em seguida digite o código que aparecerá na tela para confirmar.</OAuthDescription>
            <FloatingLabelInputContainer>
              <Form ref={formRef} onSubmit={handleTFACode} id="editForm">
                <FloatingLabel
                  htmlFor="OAuthCode"
